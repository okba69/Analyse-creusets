...
def detect_sets_and_anomalies(df: pd.DataFrame):
    ...
    # Première passe : détecter tous les sets
    ...
    for idx in range(len(df)):
        ...
        if not in_set:
            if can_detect_new_set and cnt80 >= 40:
                ...
                # Vérifier si on est sur une ligne de set ou une ligne avant/après
                is_forbidden_line = (
                    idx in set_rows or  # Ligne actuelle est un set
                    (idx + 1 < len(df) and (idx + 1) in set_rows) or  # Ligne suivante est un set
                    (idx > 0 and (idx - 1) in set_rows)  # Ligne précédente est un set
                )
                
                if not is_forbidden_line:
                    for ci, col in enumerate(data_cols):
