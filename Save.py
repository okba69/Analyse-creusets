...
def get_shift(t: time) -> str:
    if time(5,0) <= t < time(13,0):   return "Matin"
    elif time(13,0) <= t < time(21,0): return "Après‑midi"
    else:                               return "Nuit"
    ...
    for idx in range(len(df)):
        ...
        if not in_set:
            if cnt80 >= 40:
                ...
                meta.append({"Set": set_count, "Date": date, "Jour": day})
                last = set_count
                current = set()
                in_set = True
            else:
...
def to_excel(df, set_starts, anomaly_cells, meta, byset, top10):
    ...
    ws2.append(["Set", "Date", "Jour", "Nb anomalies"])
    for m in meta:
        s = m["Set"]
        nb = len(byset.get(s, []))
        ws2.append([s, m["Date"], m["Jour"], nb])
...
if uploaded and analyse:
    ...
    all_anoms = [x for vals in byset.values() for x in vals]
    top10 = Counter(all_anoms).most_common(10)
    top5 = top10[:5]  # on prend les 5 premiers
    
    # Affichage récapitulatif des sets
    recap = pd.DataFrame([
        {"Set": m["Set"], "Date": m["Date"], "Jour": m["Jour"], "Nb anomalies": len(byset.get(m["Set"], []))}
        for m in meta
    ]).set_index("Set")
